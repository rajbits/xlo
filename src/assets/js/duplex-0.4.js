(function()
{
	...
	var real_object = function ro(scope, s)
	{
		var ar = s.split('.'), node = scope, attr;
		$.each(ar, function(i, ss)
		{
			if (i == ar.length - 1)
			{
				attr = ss;
	...
	var Handlers =
	{
		...
		{
			var r = ro(context);
			var node = r.node, attr = r.attr, reg = /(\w*)\s*in\s*(\w*)/g;
			var p = reg.exec(attr);
			if (p == null || p.length != 3)
			{
				console.log('Expecting an array to iterate in format: x in y');
				return;
			}
